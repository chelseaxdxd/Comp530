...
int main()
{
	...
	// UNIT TEST 1: A BIG ONE!!
	{
		...
		// cout << "##unit\t\t"
		// 	 << "handle refcnt" << pinnedPage->position->refCnt << endl;
		// char *bytes = (char *)pinnedPage->getBytes();
		// writeNums(bytes, 64, 0);
		// cout << "##unit\t\t"
		// 	 << "0:bytes testing=====" << string(bytes) << endl;
		...
		// 	cout << "##unit\t\t" << i << ":bytes testing=====" << string(bytes) << endl;
		// 	temp->wroteBytes();
		// 	myHandles.push_back(temp);
		// }
		// cout << "##unit\t\t"
		// 	 << "<now forget the pages we created>" << endl;
		...
		for (int i = 0; i < 10; i++)
		{
			...
			writeNums(bytes, 64, i);
			cout << "##unit\t\t"
				 << i << ":bytes testing===1===" << string(bytes) << endl;
			cout << "##unit\t\t"
				 << "char in buff === " << oneHandle->position->bufferItemPtr->pageData[0] << endl;
			cout << "##unit\t\t"
				 << "slot num " << oneHandle->position->pageNum << endl;
			cout << "##unit\t\t"
				 << "slot num in buff " << oneHandle->position->bufferItemPtr->pageNum << endl;
			oneHandle->wroteBytes();
			cout << "allocating pinned page\n";
			MyDB_PageHandle twoHandle = myMgr.getPinnedPage();
			writeNums(bytes, 64, i);
			cout << "##unit\t\t"
				 << i << ":bytes testing===2===" << string(bytes) << endl;
			cout << "##unit\t\t"
				 << "char in buff === " << twoHandle->position->bufferItemPtr->pageData[0] << endl;
			cout << "##unit\t\t"
				 << "slot num " << twoHandle->position->pageNum << endl;
			cout << "##unit\t\t"
				 << "slot num in buff " << twoHandle->position->bufferItemPtr->pageNum << endl;
	...
	// 		cout << i << " answer " << string(answer) << endl;
	// 		cout << i << " bytes " << string(bytes) << endl;
	// 	}
	// }
}
#endif
